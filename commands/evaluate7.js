...
module.exports = {
    ...
    execute(msg, args) {
        ...
        if (msg.member.roles.cache.some(role => role.id === Data.nominationId)) {
            ...
            if (args.length != 8) {
                ...
                .setDescription(`You must enter **7 scores** and a [**link**](https://empty) with the **${Data.prefix}evaluate7** command to evaluate if a pet enters the ${Data.hallMention}!`)
                .setFooter(Data.footer.text, Data.footer.image);

                msg.channel.send(embed);

            // if invalid scores are supplied
            } else if (!module.exports.checkInput7(args[0], args[1], args[2], args[3], args[4], args[5], args[6])) {
                const embed = new Discord.MessageEmbed()
                .setColor("#DD2E44")
                .setTitle(":exclamation: **━━━━━ ERROR ━━━━━** :exclamation:")
                ...
                let score7 = parseFloat(args.shift());
                let link = args.shift();

                let finalEval = (score1 + score2 + score3 + score4 + score5 + score6 + score7) / 7;
    ...
    checkInput7(val1, val2, val3, val4, val5, val6, val7) {
        ...
        if (!Number.isInteger(parseInt(val1)) || !Number.isInteger(parseInt(val2)) || !Number.isInteger(parseInt(val3)) || !Number.isInteger(parseInt(val4))
            || !Number.isInteger(parseInt(val5)) || !Number.isInteger(parseInt(val6)) || !Number.isInteger(parseInt(val7)) || val1 < 0 || val1 > 5 || val2 < 0 
            || val2 > 5 || val3 < 0 || val3 > 5 || val4 < 0 || val4 > 5 || val5 < 0 || val5 > 5 || val6 < 0 || val6 > 5 || val7 < 0 || val7 > 5) {
            return false;
        }
